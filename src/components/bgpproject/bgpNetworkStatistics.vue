<template>
  <div class="grid">
    <div class="col">
      <DataTable :value="bgpProjectStatistics" responsiveLayout="scroll" :selected="isSelectedDetail" selectionMode="single"
        dataKey="id" @row-select="statisticsDetail">
        <Column field="ulkeAdi" header="Ülke Adı" :sortable="true" bodyStyle="text-align:left;">
          <template #footer>
            <b>Toplam: </b>
          </template>
        </Column>
        <Column field="wrongNumber" header="Yanlış Numara" :sortable="true" bodyStyle="text-align:left;">
          <template #footer>
            {{ bgpProjectStatisticsDetailSum.wrongNumber }}
          </template>
        </Column>
        <Column field="notResponse" header="Cevap Yok" :sortable="true" bodyStyle="text-align:left;">
          <template #footer>
            {{ bgpProjectStatisticsDetailSum.notResponse }}
          </template>
        </Column>
        <Column field="notInterested" header="İlgilenmiyor" :sortable="true" bodyStyle="text-align:left;">
          <template #footer>
            {{ bgpProjectStatisticsDetailSum.notInterested }}
          </template>
        </Column>
        <Column field="interested" header="İlgili" :sortable="true" bodyStyle="text-align:left;">
          <template #footer>
  ...
  <Dialog v-model:visible="statistics_detail" :header="ulkeAdi" maximizable :modal="true" position="top">
    <div class="grid">
      <div class="col">
        <DataTable :value="bgpProjectStatisticsDetail" responsiveLayout="scroll" v-model:filters="filters" filterDisplay="row">
          <Column field="projectName" header="Proje Adı" :showFilterMenu="false"> <template
